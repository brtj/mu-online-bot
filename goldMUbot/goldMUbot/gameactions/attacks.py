...
def round_attack(player_info, deltas, step_delay=0.005, pause_range=(0.1, 0.4), hold_time=1, level_max=100, coord_x=0, coord_y=0):
    ...
    send_message(f"ja tu tylko dobije do {level_max} lvl potem ide", player_info=player_info)
    send_message(f"pozdro, nie jestem botem (chyba)", player_info=player_info)
...
def attack_with_helper_on_spot(player_info, mouse_on_map_x, mouse_on_map_y, desired_coord_x, desired_coord_y, tolerance=8, timeout=80, print_txt="helper attack", main_player_location_name="", map_max=""):
  ...
  if is_at_position(player_data["location_coord_x"], player_data["location_coord_y"], desired_coord_x, desired_coord_y):
    if player_data["helper_status"] != "Running":
      ...
      send_message(f"/post hej zapraszam na PT patrz pod P (sub2)", player_info=player_info)
      send_message(f"/post zapraszam PT, jestem {main_player_location_name} ({desired_coord_x},{desired_coord_y}) do {map_max} lvlu", player_info=player_info)
    else:
