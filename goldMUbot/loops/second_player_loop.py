...
def second_player_loop():
    ...
    second_player_location_name = (
        second_player_data.get("location_name") or "not_available"
    )
    ...
    if second_player_level == 400:
        ...
        STATE_SECOND_PLAYER.update_dict(
            "second_player_data",
    ...
    if second_player_level == 1 and not stats_added:
        ...
        second_player_add_stats(
            player_info=second_player_name, second_player_reset=second_player_reset
        )
        check_if_its_in_party(player_info=second_player_name)
    ...
    if second_player_level == 1 and run_speedrun and not is_it_speedrun:
        ...
        STATE_SECOND_PLAYER.update_dict(
            "second_player_data", {"is_it_speedrun": True, "run_speedrun": False}
        )
    ...
    if (
        devias_enabled
        and devias_max >= second_player_level >= devias_min
        and second_player_location_name == "Devias"
    ):
    ...
    if main_player_level >= atlans2_min and second_player_level >= atlans2_min:
        generic_attack_on_spot(
            map_enabled=atlans_enabled,
            map_name="Atlans",  # map_name
    ...
    if main_player_level >= icarus2_min and second_player_level >= icarus2_min:
        generic_attack_on_spot(
    ...
    if main_player_level >= aida_min and second_player_level >= aida_min:
        generic_attack_on_spot(
    ...
    if main_player_level >= karutan2_min and second_player_level >= karutan2_min:
        generic_attack_on_spot(
    ...
    if main_player_level >= lacleon_min and second_player_level >= lacleon_min:
        generic_attack_on_spot(
