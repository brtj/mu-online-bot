...
from gameactions.attacks import attack_no_helper_on_spot, primary_attack_no_helper_till_80lvl
...
def main_player_loop(state):
    ...
    if main_player_level == 400:
        ...
        STATE.update_dict(
            "main_player_data",
            {"stats_added": False, "is_it_speedrun": False, "run_speedrun": False},
        )
        ...
        if type_game == "solo":
            ...
            if 400 > second_player_level > 370:
                logger.info(
                    "Doing nothing, waiting for second player to reach reset level..."
                )
            else:
                jewels_to_bank(player_info=main_player_name)
                elf_reset(player_info=main_player_name, reset_count=reset_count)
    ...
    if main_player_level == 1 and not stats_added:
        ...
        main_player_add_stats(
            player_info=main_player_name, main_player_reset=main_player_reset
        )
    ...
    if main_player_level == 1 and run_speedrun and not is_it_speedrun:
        ...
        STATE.update_dict(
            "main_player_data", {"is_it_speedrun": True, "run_speedrun": False}
        )

    if type_game == "two_players_in_party":
        second_player_data = STATE_SECOND_PLAYER.get("second_player_data") or {}
        second_player_level = int(second_player_data.get("level") or 0)
        if second_player_level >= 80:
            primary_attack_no_helper_till_80lvl(
                primary_enabled=primary_enabled,
                primary_map_name=primary_map_name,
                primary_min=primary_min,
                primary_max=primary_max,
                main_player_level=main_player_level,
                main_player_name=main_player_name,
                main_player_location_x=main_player_location_x,
                main_player_location_y=main_player_location_y,
                main_player_location_name=main_player_location_name,
                devias_enabled=devias_enabled,
                devias_min=devias_min,
                devias_max=devias_max,
            )
        else:
            logger.info("Second player level is %s, waiting to reach 80 for primary attack...", second_player_level)
    else:
        primary_attack_no_helper_till_80lvl(
            primary_enabled=primary_enabled,
            primary_map_name=primary_map_name,
    ...
    # atlans2 -----------------------------------------
    atlans2_spot = (main_player_data.get("map_spots") or {}).get("atlans_map_spots")
    ...
    generic_attack_on_spot(
        ...
        map_min=atlans2_min,  # lvl_min due to teleport case
        player_name=main_player_name,
        player_level=main_player_level,
        player_location_name=main_player_location_name,
        player_location_x=main_player_location_x,
