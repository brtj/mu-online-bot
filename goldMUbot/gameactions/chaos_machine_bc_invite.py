...
def chaos_machine_bc_invite(player_info):
    ...
    payload_inventory_box_status = post(
        HOSTAPI_ENDPOINTS["find_and_hover"], payload_inventory_box
    )
    time.sleep(0.7)
    if payload_inventory_box_status.get("ok") == True:
        ...
        payload_blood_bone7_box_status = post(
            HOSTAPI_ENDPOINTS["find_and_hover"], payload_blood_bone7_box
        )
        ...
        payload_scroll_of_arch7_box_status = post(
            HOSTAPI_ENDPOINTS["find_and_hover"], payload_scroll_of_arch7_box
        )
