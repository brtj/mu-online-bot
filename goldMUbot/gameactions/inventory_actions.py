...
def jewels_to_bank(player_info=None):
    ...
    if jewel_inventory_status.get("ok") == True:
        ...
        jewel_inventory_all_icon_status = post(
            HOSTAPI_ENDPOINTS["find_and_hover"], payload
        )
        logger.info(jewel_inventory_all_icon_status)
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_1st_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_2nd_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_3rd_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_4th_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_5th_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_6th_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_7th_all_icon"
        )
        jewels_click_on_all(
            player_info=player_info, hud_coords_id="jewel_bank_8th_all_icon"
        )
        press_escape()
        press_escape()
