...
def round_attack(
    ...
    while current_level <= level_max:
        ...
        if not is_at_position(location_x, location_y, coord_x, coord_y, tol=tol):
            logger.warning(
                "[SAFE EXIT] Player not at expected position — aborting loop"
            )
            logger.info(
                f"SAFE EXIT details: location_x={location_x}, location_y={location_y}, expected_x={coord_x}, expected_y={coord_y}, tol={tol}"
            )
            break
...
def attack_no_helper_on_spot(
    ...
    delta,
    level_max=100,
    print_txt="no helper attack",
):
    if is_at_position(
        location_coord_x, location_coord_y, desired_coord_x, desired_coord_y, tol=20
    ):
        round_attack(
            player_info=player_info,
            hold_time=3,
            level_max=level_max,
            coord_x=desired_coord_x,
            coord_y=desired_coord_y,
            deltas=delta,
            tol=20,
        )
    else:
        go_to_point_and_wait(
            player_info=player_info,
            mouse_x=mouse_on_map_x,
            mouse_y=mouse_on_map_y,
            target_loc_x=desired_coord_x,
            target_loc_y=desired_coord_y,
            print_txt=print_txt,
        )


def attack_with_helper_on_spot(
    player_info,
    mouse_on_map_x,
    mouse_on_map_y,
    desired_coord_x,
    ...
    if is_at_position(
        ...
        if check_helper_status == "Not running":
            ...
            if send_message:
                message = generate_mu_party_message(
                    player_location_name, desired_coord_x, desired_coord_y
                )
